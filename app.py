...
def admin_dashboard():
    ...
    week_end = (monday + timedelta(days=4)).date()  # Friday
    ...
    try:
        ...
        if not counselors:
            flash("No counselors found.", "warning")

        # Query classes for the week — FORCE date comparison as proper dates
        c.execute("""
        ...
        logger.error(f"admin_dashboard crashed: {e}", exc_info=True)
